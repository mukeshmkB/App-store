...
class AppStore extends Component {
  ...
  render() {
    ...
    const filteredApp = appsList.filter(
      eachElement => eachElement.category === activeTabId,
    )
    const userFilterApp = filteredApp.filter(eachElement =>
      eachElement.appName.toLowerCase().includes(searchInput.toLowerCase()),
    )
    return (
        ...
        <div className="app-store-container">
          ...
          <ul className="tabs-list-container">
            {tabsList.map(eachElement => (
              <TabItem
                tabsDetails={eachElement}
                key={eachElement.tabId}
                updatActiveTabId={this.updatActiveTabId}
                isActive={eachElement.tabId === activeTabId}
              />
            ))}
          </ul>
          <ul className="apps-container">
            {userFilterApp.map(eachElement => (
              <AppItem appsDetails={eachElement} key={eachElement.appId} />
            ))}
          </ul>
        </div>
      </div>
